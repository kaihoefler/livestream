...
export default {
  ...
  computed: {
    ...
    storedDeadline () {
      return this.$store.state.displayControl.countdown.deadline
    },
    seconds () {
  ...
  watch: {
    deadline: function (newVal, oldVal) {
      ...
      this.date = Math.trunc(Date.parse(endTime.replace(/-/g, '/')) / 1000)
      if (!this.date) {
        throw new Error("Invalid props value, correct the 'deadline': " + this.deadline)
      }
    },
    now (value) {
    ...
    updateTime (idx, newValue) {
      ...
      if (val !== d.current) {
        ...
        if (el) {
          ...
          if (el.offsetWidth === 999999999 && 1 === 0) {
            console.log('I can not be here')
          }
